...
public class DrawTool extends Subject {
    ...
    private float textSymbolSize = 12;
    private String textSymbolText = "";
...
//    private FillSymbol circleFillSymbol;
    ...
    private PointBuilder pointGeometry;
    private EnvelopeBuilder envelopeGeometry;
    private PolylineBuilder polylineGeometry;
    private PolygonBuilder polygonGeometry;
...
//            case DrawTool.CIRCLE:
            ...
            case DrawTool.POLYLINE:
                Log.d(TAG, "activate: polyline");
            case DrawTool.FREEHAND_POLYLINE:
                Log.d(TAG, "activate: free polyline");
                this.polylineGeometry = new PolylineBuilder(getSpatialReference());
                clear();//清除顶点和节点信息
                break;
        }
    ...
    public void setTextSymbolSize(float textSymbolSize) {
        this.textSymbolSize = textSymbolSize;
    }

    public void setTextSymbolText(String textSymbolText) {
        this.textSymbolText = textSymbolText;
    }
    ...
    public void setEditGraphic(Graphic graphic){
        ...
        if(geometry!=null){
           ...
           if(type== GeometryType.POINT||type== GeometryType.MULTIPOINT){//点
                //不执行任何操作
           }else if(type== GeometryType.POLYLINE){//线

               this.activate(DrawTool.POLYLINE);
               //获取点并按指定显示在地图上
               Graphic graphic1 = new Graphic(geometry,DrawSymbol.mLineSymbol);
//               graphicID =this.mGraphicsLayerEditing.getGraphics().add(graphic1);
               this.mGraphicsLayerEditing.getGraphics().add(graphic1);
               this.drawGraphic = graphic1;//保存当前
               Multipart polyline = (Multipart)graphic1.getGeometry();
               this.mPoints.clear();
               for (int i =0;i<polyline.getParts().get(0).getPointCount();i++){
                   Point pt = polyline.getParts().get(0).getPoint(i);
                   this.mPoints.add(pt);
               }
               refresh();

           }else if(type== GeometryType.ENVELOPE||type== GeometryType.POLYGON){//面
...
//               graphicID =this.mGraphicsLayerEditing.addGraphic(graphic1);
               this.mGraphicsLayerEditing.getGraphics().add(graphic1);
               this.drawGraphic = graphic1;//保存当前
               Multipart polygon = (Multipart)graphic1.getGeometry();
               this.mPoints.clear();
               for(int i =0;i<polygon.getParts().get(0).getPointCount();i++){
                    Point pt = polygon.getParts().get(0).getPoint(i);
                    this.mPoints.add(pt);
               }
               refresh();

           }else if(type== GeometryType.UNKNOWN){//未知
...
//    }
    ...
    private void drawVertices() {
        ...
        for (Point pt : mPoints) {
            if (mVertexSelected && index == mInsertingIndex) {
                ...
                symbol = DrawSymbol.mYelloMarkerSymbol;
            }
    ...
    private void drawPolylineOrPolygon() {
        if (mPoints.size() >= 1) {
            //创建多段线或者面
            if (drawType == POLYLINE || drawType == FREEHAND_POLYLINE ) {
                this.drawGraphic = new Graphic(polylineGeometry.toGeometry(), lineSymbol) ;
            } else if(drawType == POLYGON || drawType == FREEHAND_POLYGON) {
                this.drawGraphic = new Graphic(polygonGeometry.toGeometry(), fillSymbol);
            }else if(drawType == ENVELOPE){
                this.drawGraphic = new Graphic(envelopeGeometry.toGeometry(), lineSymbol) ;
            }
...
//    }
    ...
    class DrawTouchListener extends DefaultMapViewOnTouchListener {
        ...
        public DrawTouchListener(Context context, MapView view) {
            super(context, view);
            Log.d(TAG, "DrawTouchListener: init");
        }
            ...
            if (active && ( drawType==POINT || drawType == POLYGON || drawType == POLYLINE || drawType == TEXT || drawType == PICTURE)){
                ...
                switch (drawType) {
                    ...
                    case DrawTool.POLYGON:
                        ...
                        break;
                    case DrawTool.POLYLINE:
                        polylineGeometry.addPoint(point);
                        mPoints.add(point);
                        refresh();
                        break;
                }
...
//                        break;
                    case FREEHAND_POLYGON:
                        if(event.getAction() == MotionEvent.ACTION_DOWN){
                            ...
                            drawGraphic = new Graphic(polygonGeometry.toGeometry(),fillSymbol);
                            isCompleteDraw=true;
                            sendDrawEndEvent();
                            clear();
                        }
                    ...
                    case FREEHAND_POLYLINE:
                        if(event.getAction() == MotionEvent.ACTION_UP){
                            drawGraphic = new Graphic(polylineGeometry.toGeometry(),lineSymbol);
                            isCompleteDraw=true;
                            sendDrawEndEvent();
                            clear();
                        }else{
                    ...
                    case ENVELOPE:
                        if(event.getAction() == MotionEvent.ACTION_DOWN){
                            ...
                            double minx = startPoint.getX() > point.getX() ? point
                                    .getX() : startPoint.getX();
                            double miny = startPoint.getY() > point.getY() ? point
                                    .getY() : startPoint.getY();
                            double maxx = startPoint.getX() < point.getX() ? point
                                    .getX() : startPoint.getX();
                            double maxy = startPoint.getY() < point.getY() ? point
                                    .getY() : startPoint.getY();
                            envelopeGeometry = new EnvelopeBuilder(new Envelope(new Point(minx, miny), new Point(maxx, maxy)));
                            refresh();
                        }else if(event.getAction() == MotionEvent.ACTION_UP){
                            drawGraphic = new Graphic(envelopeGeometry.toGeometry(),lineSymbol);
                            isCompleteDraw=true;
                            sendDrawEndEvent();
                            clear();
                        }
        ...
        private boolean handleTap(MotionEvent event) {
            ...
            if (active && (drawType==POLYGON || drawType==POLYLINE)) {
                ...
                DrawTool.this.pointGeometry.setXY(point.getX(), point.getY());
                sendDrawEndEvent();
                isCompleteDraw=true;//开始绘制
                return true;
            }
        ...
        private int getSelectedIndex(double x, double y, ArrayList<Point> points, MapView map) {
            ...
            for (int i = 0; i < points.size(); i++) {
                ...
                android.graphics.Point p = map.locationToScreen(points.get(i));
                double diffx = p.x - x;
                double diffy = p.y - y;
